...
const REFERENCE_REGEX = /^KF-\d+$/i;

/**
 * Normalizes user input into a `KF-####` reference.
 * - `14`        → `KF-0014`
 * - `KF-14`     → `KF-0014`
 * - `KF-0014`   → `KF-0014`
 * - `kf-0014`   → `KF-0014`
...
function normalizeReference(input: string): string {
  ...
  // Already in KF-XXXX form → normalize the numeric part
  if (trimmed.startsWith("KF-")) {
    const numeric = trimmed.slice(3).replace(/\D/g, "");
    return numeric ? `KF-${numeric.padStart(4, "0")}` : "";
  }

  // Digits only → auto-format
  if (/^\d+$/.test(trimmed)) {
    return `KF-${trimmed.padStart(4, "0")}`;
  }
...
export default function TrackOrder() {
  ...
  /** Tracks the currently-searched reference so polling always uses it. */
  const activeReferenceRef = useRef<string | null>(null);

  /** Monotonic token — the latest request wins; older responses are dropped. */
  const requestTokenRef = useRef(0);

  /** Last seen status, for change notifications. */
  ...
  const requestNotificationPermission = async () => {
    ...
    if (perm === "granted") {
      setNotificationsEnabled(true);
      toast.success(
        "Live Order Notifications Enabled! You will be alerted on status changes.",
      );
    } else {
  ...
  const fetchOrder = useCallback(
    async (query: string, isSilentRefresh = false) => {
      const reference = normalizeReference(query);

      // Guard: empty / invalid reference
      if (!reference) {
        setError("Please enter a valid Order ID (e.g. KF-0001).");
        setOrder(null);
      ...
      if (!REFERENCE_REGEX.test(reference)) {
        setError(
          `Invalid reference "${query}". Please use the format KF-0001.`,
        );
        setOrder(null);
        activeReferenceRef.current = null;
        lastStatusRef.current = null;
      ...
      // Mark this as the active reference — polling uses this
      activeReferenceRef.current = reference;

      // Token for out-of-order response protection
      ...
      try {
        const res = await apiGet<{
          ...
          data?: BackendOrder;
          message?: string;
        }>(`/api/orders/track/${encodeURIComponent(reference)}`);

        // Stale response — a newer request superseded us
        if (token !== requestTokenRef.current) return;

        // Explicit failure → clear the previous order, even on silent refresh
        if (!res?.success || !res.data) {
          setError(
            `No order found for reference "${reference}". Please verify your tracking code.`,
          );
          setOrder(null);
          lastStatusRef.current = null;
          return;
        ...
        const foundBackendOrder = res.data;
        const status =
          STATUS_MAP[(foundBackendOrder.status || "").toLowerCase()] ||
          "Pending";
        const { date, time } = formatOrderDate(foundBackendOrder.order_date);

        // Prefer the stored reference, fall back to the requested one
        const formattedId =
          foundBackendOrder.order_reference || reference;
        ...
        const tracked: TrackedOrder = {
          ...
          customerName:
            [
              foundBackendOrder.customer_first_name,
              foundBackendOrder.customer_last_name,
            ]
              .filter(Boolean)
              .join(" ") ||
          ...
          orderTime: time,
          paymentMethod:
            foundBackendOrder.order_source === "externalorder"
              ? "Cash on Delivery"
              : "MTN Mobile Money",
          paymentStatus:
        ...
        try {
          ...
          // Stale response check again
          if (token !== requestTokenRef.current) return;

          const mappedItems: DisplayOrderItem[] = (itemsRes?.data || []).map(
            (it) => ({
              id: it.id,
              ...
              subtotal:
                Number(it.subtotal) ||
                Number(it.quantity * it.unit_price) ||
                0,
          ...
          setOrder((prev) =>
            prev && prev.rawId === foundBackendOrder.id
        ...
        const msg =
          err instanceof Error
            ? err.message
            : "Could not fetch order tracking details.";

        // Always clear on error, whether silent or not
        setError(msg);
        setOrder(null);
        lastStatusRef.current = null;
      } finally {
        if (token === requestTokenRef.current && !isSilentRefresh) {
          setLoading(false);
        }
      }
    },
  ...
  // Load on mount / when URL id changes
  useEffect(() => {
    if (initialId) {
  ...
  // Auto-poll ONLY when an order is actually loaded AND has a valid reference
  useEffect(() => {
    if (!autoRefresh) return;
    if (!order?.formattedId) return;

    const interval = setInterval(() => {
      fetchOrder(order.formattedId, true);
    }, 15000);

    return () => clearInterval(interval);
  }, [autoRefresh, order?.formattedId, fetchOrder]);
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    ...
    const reference = normalizeReference(trimmed);

    // Clear previous state before search
    setOrder(null);
    setError(null);
    lastStatusRef.current = null;

    // Reflect in URL so refresh/share keeps the same reference
    setSearchParams({ id: reference });

    // If the URL didn't actually change (same value), fetch manually
    if (reference === initialId) {
    ...
    <div className="min-h-screen flex flex-col bg-[#FFFDF9] text-[#1F2937]">
      ...
      <main className="flex-1 pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto w-full">
        ...
        <div className="text-center max-w-2xl mx-auto mb-10">
          ...
          >
            <Truck size={14} className="text-[#076935]" /> Live Order
            Tracking
          </span>
          <h1
          ...
          <p className="mt-2 text-sm sm:text-base text-gray-600">
            Real-time cold-chain visibility from organic farms in Rwanda
          ...
          >
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Enter Order ID (e.g. KF-0001)..."
            ...
            <div className="bg-white p-6 sm:p-8 rounded-3xl border border-[#076935]/15 shadow-sm">
                ...
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-gray-500 mt-1 flex items-center gap-3 flex-wrap">
                    <span className="inline-flex items-center gap-1">
                      <Calendar size={13} className="text-[#F39927]" /> Placed
                      on {order.orderDate} at {order.orderTime}
                    </span>
                    <span>•</span>
                    <span className="inline-flex items-center gap-1">
                      <MapPin size={13} className="text-[#F39927]" />{" "}
                      {order.district}
                    </span>
                  </p>
                </div>
              ...
              <div className="pt-8 pb-4">
                <div className="relative flex items-center justify-between">
                  ...
                  {[
                    ...
                    {
                      title: "Processing",
                      desc: "Harvest & Cold Storage",
                      icon: <Package size={18} />,
                    },
                    {
                      title: "Shipped",
                      desc: "Temperature Transit",
                      icon: <Truck size={18} />,
                    },
                    {
                      title: "Delivered",
                    ...
                    return (
                      <div
                        key={step.title}
                        className="relative z-10 flex flex-col items-center group text-center max-w-[120px]"
                      >
                        <div
              ...
              <div className="mt-6 p-4 rounded-2xl bg-[#F4FAF7] border border-[#076935]/15 flex items-center justify-between gap-4 flex-wrap">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      Your produce is maintained between 4°C - 8°C throughout
                      transport.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => fetchOrder(order.formattedId)}
                    disabled={loading}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 bg-white p-6 rounded-3xl border border-[#076935]/15 shadow-sm">
                <div className="flex items-center justify-between pb-4 border-b border-gray-100 mb-4">
                  <h3
                    ...
                    style={{ fontFamily: "var(--font-heading)" }}
                  >
                    <ShoppingBag size={18} className="text-[#076935]" /> Ordered
                    Organic Items
                  </h3>
                  <span className="text-xs font-semibold text-gray-500">
                    {order.items.length} produce{" "}
                ...
                {itemsLoading ? (
                  ...
                  <div className="py-8 text-center text-gray-500">
                    ...
                    <p className="text-sm font-semibold">
                      Standard Organic Harvest Parcel
                    </p>
                    <p className="text-xs text-gray-400">
                      Line item breakdown loaded upon dispatch.
                    </p>
                  </div>
                ) : (
                    ...
                    <table className="w-full text-left border-collapse">
                      ...
                      <tbody className="divide-y divide-gray-50 text-sm">
                        {order.items.map((item) => (
                          <tr
                            key={item.id}
                            className="hover:bg-[#F4FAF7]/40 transition"
                          >
              ...
              <div className="space-y-6">
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-gray-500 flex items-center gap-1.5">
                        <CreditCard size={14} className="text-[#076935]" />{" "}
                        Method
                      </span>
                ...
                <div className="bg-[#F4FAF7] p-5 rounded-3xl border border-[#076935]/20 text-center">
                  <HelpCircle
                    size={24}
                    className="mx-auto mb-2 text-[#076935]"
                  />
                  <h4
                  ...
                  <p className="text-xs text-gray-600 mt-1 mb-4">
                    Our customer support team is on standby to assist with
                    delivery instructions or inquiries.
                  </p>
                  <Link
                    to="/contact"
